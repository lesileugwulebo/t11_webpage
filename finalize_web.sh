...
sudo apt-get install -y nodejs git nginx python3 python3-pip
...
if [ ! -d /var/www/inventory-app/.git ]; then
    git clone https://github.com/lesileugwulebo/t11_webpage.git /var/www/inventory-app
else
    cd /var/www/inventory-app && git fetch origin && git reset --hard origin/main
fi
...
echo "=== All Services Running on GCP! ==="
